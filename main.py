...
path = "C:\\Users\\hotpot\\Desktop\\LearnCode\\git\\python_climate_git\\function_new\\"
path2 = "E:\\1979-1989\\"
...
for y in range(1979, 1980):
    for m in range(1, 13):  # 遍历月
        ...
        ds = xr.open_mfdataset(path2 + str(y) + str(m).zfill(2) + '.nc')

        # 将时间维度从小时转换为天
        ds = ds.resample(time='1D').sum(dim='time')

        # 保存为新的.nc文件
        ds.to_netcdf(path2 + str(y) + str(m).zfill(2) + '_day.nc')
        ds.close()
        # 定义阈值
        ...
        # 保存为新的.nc文件
np.save(path + '_wetday.npy', wet_day)
end_time = time.time()
