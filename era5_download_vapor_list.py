...
if __name__ == '__main__':
    ...
    dic = {
        ...
        'format': 'netcdf',  # 数据格式
        'variable': 'total_column_water_vapour',  # 变量名称
        'year': '',  # 年，设为空
        'month': '',  # 月，设为空
        'day': [],  # 日，设为空
        'time': [  # 小时
    ...
    for y in range(1980, 2020):  # 遍历年
        for m in range(1, 13):  # 遍历月
            ...
            # path = 'F:\\liusch\\ERA5\\1980-2019\\total_column_water_vapour'  # 存放文件夹
            # filename = str(y) + str(m).zfill(2) + '.nc'  # 文件名
            # idmDownloader(url, path, filename)  # 添加进IDM中下载

            # 添加进自定义队列
            queue_path = 'F:\\liusch\\IDM\\queue.txt'
