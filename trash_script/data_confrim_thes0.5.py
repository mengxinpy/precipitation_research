...
for y in range(1979, 1980):
    for m in range(1, 13):  # 遍历月
        ...
        # 将时间维度从小时转换为天
        dsr = ds.resample(time='1D').sum(dim='time')

        # 保存为新的.nc文件
        # ds.to_netcdf(path + str(y) + str(m).zfill(2) + '_day.nc')
        # 定义阈值
        threshold = 0.0005  # 需要根据实际情况设定阈值

        # 使用where()函数创建掩码矩阵
        mask = dsr > threshold
        # for i in range(31):
        #     draw_area_heap(mask.astype(int).to_array().values[0, i, :, :], i)
        # 沿着时间维度求和
        monthly_sum = mask.sum(dim='time')
        wet_day = wet_day + np.squeeze(monthly_sum.to_array().values)
        draw_area_heap(wet_day, 'month_wet_thes0.5'+str(m))
        ds.close()
