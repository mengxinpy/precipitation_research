...
# 注意初始值要乘1000
path = "F:\\liusch\\morph_links\\"  # path2 = "E:\\1979-1989\\" start_time = time.time()
# 定义降雨量的bins
rainfall_bins = np.logspace(np.log10(1), np.log10(500), num=500)  # 对数尺度的bins
# rainfall_frequency = np.squeeze(np.load('paper.npy')) / 100
rainfall_frequency = np.squeeze(xr.open_dataset('cmporph_process_12.nc').to_array().values)
...
ds = np.squeeze(xr.open_mfdataset(path + '*.nc')['cmorph'].values)
...
np.save('cmorph_percentile', result)
