...
nc_files = [os.path.join(path, f) for f in os.listdir(path) if fnmatch.fnmatch(f, '*_processed_day_1.nc') and '_processed_day_processed_day' not in f]
...
cmorph_process.to_netcdf('era5_frequency_1.nc')
...
np.save('ear5_percentile_area_1deg_6area.npy', result_percentile)
end_time = time.time()
