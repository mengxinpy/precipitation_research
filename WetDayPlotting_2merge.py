...
infile2 = xarray.open_dataset('F:\\liusch\\remote_project\\climate_new\\imerg5_process.nc')['precipitationCal'].sel(lat=slice(-60, 60)) * 100
...
dist = np.load('cmorph_percentile.npy')
dist2 = np.load('imerg_percentile_area.npy')
# dist = np.load(path + 'Figure1\\total_precipitation_distribution.npy')
dist_arr = np.asarray(dist)
dist_arr2 = np.asarray(dist2)
...
cont = plt.contourf(infile.lon, infile.lat, infile, levels=20, cmap=cmap, vmin=0, vmax=100)
# 设置地图的范围
ax.set_global()
...
# ax.add_feature(cartopy.feature.LAND, facecolor='white')
# ax.add_feature(cartopy.feature.OCEAN, facecolor='lightblue')
cont2 = plt.contourf(infile2.lon, infile2.lat, infile2.transpose(), levels=20, cmap=cmap, vmin=0, vmax=100)
# 设置地图的范围
ax.set_global()
# setting axes and labels for subplot 2
...
for ind, d in enumerate(dist_arr2[0:100, :]):
    if str(type(d)) == "<class 'float'>":  # this statement ignores data that doesn't exist.
        None
    else:
        plt.plot(d, np.arange(0, 100, 1), '.', color=colors[ind], markersize=10)
