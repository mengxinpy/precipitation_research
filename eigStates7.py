...
for a in range(0, 4):
    ...
    for i in range(out_num):
        for v in ['vapor', 'rain']:
            ...
            np.save('decode_' + v + str(a) + '_7.2th', decode_out_variables)
            ...
            save_path = 'pcaPicture7.2th\\' + area_name_list[a] + '\\' + area_name_list[a] + '_' + str(i) + 'th_' + v + '.png'
            os.makedirs(os.path.dirname(save_path), exist_ok=True)
            plt.savefig(save_path)
            plt.close()
np.save('singular_rain_7.2.npy', singular_list_rain)
np.save('singular_vapor_7.2.npy', singular_list_vapor)
end_time = time.time()  # 记录结束时间
run_time = end_time - start_time  # 计算运行时间
print("程序运行时间为：", run_time, "秒")
