...
for a in range(0, 4):
    ...
    filtered_normalized_vapor = np.nan_to_num(square_normalize(filtered_elements_normalized_vapor))
    filtered_normalized_rain = np.nan_to_num(square_normalize(filtered_elements_normalized_rain))
    ...
    # print('hah')
    # hist, bin_edges = np.histogram(np.nan_to_num(filtered_elements[0],nan=-5), 100)
    # plt = sns.displot(filtered_elements[0].flatten())
    # plt.show()
    ...
    for i in range(out_num):
        for v in ['vapor', 'rain']:
            ...
            np.save('decode_' + v + str(a) + '_7.3th', decode_out_variables)
            ...
            # 画热度图
            # levels = np.logspace(np.min(decode_out_variables), np.max(decode_out_variables)
            vmin = np.nanmin(decode_out_variables)
            vmax = np.nanmax(decode_out_variables)
            if v == 'vapor':
                levels = np.linspace(vmin, vmax, 100)
            else:
                levels = np.logspace(np.log10(np.abs(decode_out_variables).nanmin()), np.log10(np.abs(decode_out_variables).nanmax()), 100)
            if start_lon < end_lon:
                lon = np.linspace(start_lon, end_lon, global_var[1])
                c = ax.contourf(lon, lat, decode_out_variables, transform=ccrs.PlateCarree(), levels=levels, cmap='RdYlBu_r')
            ...
            save_path = 'pcaPicture7.3th\\' + area_name_list[a] + '\\' + area_name_list[a] + '_' + str(i) + 'th_' + v + '.png'
