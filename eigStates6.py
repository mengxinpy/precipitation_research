...
def square_normalize(matrix):
    matrix = np.where((0 <= matrix) & (matrix <= 250), matrix, np.nan)
...
def my_svd(m, n):
    ...
    u = svd.fit_transform(k)
    return u, svd.singular_values_
...
for a in range(0, 4):
    ...
    # 找出有大量异常值点的天数去掉
    eox_indices = np.unique(np.argwhere(np.sum(filtered_elements[0, :, :] < 0, axis=0) > 3000))
    ...
    reduced_matrix_vapor, singular_list_vapor[a, :] = my_svd(filtered_normalized_vapor, out_num)
    reduced_matrix_rain, singular_list_rain[a, :] = my_svd(filtered_normalized_rain, out_num)
    np.save('reduce_matrix_vapor_' + str(a) + 'th', reduced_matrix_vapor)
    np.save('reduce_matrix_rain' + str(a) + 'th', reduced_matrix_rain)
    # 进行奇异值分解并将数据保存
    ...
    for i in range(out_num):
        for v in ['vapor', 'rain']:
            if v == 'vapor':
                ...
                np.save('decode_' + v + str(a) + '_7th', decode_out_variables)
                # 创建图形和坐标轴
            ...
            if start_lon < end_lon:
                ...
                c = ax.contourf(lon, lat, decode_out_variables, transform=ccrs.PlateCarree(), cmap='RdYlBu_r')
            ...
            save_path = 'pcaPicture7th\\' + area_name_list[a] + '\\' + area_name_list[a] + '_' + str(i) + 'th_' + v + '.png'
            os.makedirs(os.path.dirname(save_path), exist_ok=True)
            plt.savefig(save_path)
            plt.close()
np.save('singular_vapor_7.npy', singular_list_vapor)
np.save('singular_rain_7.npy', singular_list_rain)
