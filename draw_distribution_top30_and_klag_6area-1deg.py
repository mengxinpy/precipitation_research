...
all_data = np.load(f'{path_out}\\result_klag_1deg_6area.npy').transpose((1, 0, 2))

fig = plt.figure(figsize=(10, 10))
fig.suptitle(f'era5_1deg_40year', fontsize=18)
...
for area_num, area_data in enumerate(all_data):
    ...
    for ind, p in enumerate(area_data):
        if ind == 0:
            plt.plot(al[:: 2], np.arange(1, 101)[:: 2], '.', color=colors[area_num], markersize=5)

        elif ind == 1:
            plt.plot(top30[:: 2], np.arange(1, 101)[:: 2], 's', color=colors[area_num], markerfacecolor='none', markeredgecolor=colors[area_num], markersize=5)
            ...
            plt.plot(p[:: 2], np.arange(1, 101)[:: 2], '-', color=colors[area_num], markersize=10)
            print(area_num, '|', ind)
    ...
    plt.savefig(f'.\\temp_fig\\ear5_klag_area-1deg\\ear5_percentile_lag_40years_area-{area_num}-1deg.png')
    plt.close()
