...
data = np.load('era5_percentile_area_vapor_single.npy')
# data = np.load('era5_percentile_area_vapor_single_cul.npy')

fig = plt.figure(figsize=(40, 20))
fig.suptitle(f'era5_0.25_vapor_conditioned', fontsize=18)
...
for v, d_all in enumerate(data):
    ...
    plt.title(f"vapors_bins:{bins_v[v - 1] if v > 0 else 0}-{bins_v[v] if v < data.shape[0] - 1 else 'inf+'}",fontsize='25')
...
plt.savefig(f'.\\temp_fig\\cul_vapor_conditioned.png')
