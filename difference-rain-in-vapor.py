...
def select_top_30_percent(arr):
    # 计算前 30% 的阈值
    threshold = int(np.prod(arr.shape) * 0.3)

    # 获取扁平化数组的排序索引，然后反转以进行降序排序
    indices_flat_sorted = np.argsort(arr.flatten())[::-1]

    # 选择前 30% 的索引
    top_30_indices_flat = indices_flat_sorted[:threshold]

    # 将一维索引转换为二维索引
    top_30_indices_2d = np.unravel_index(top_30_indices_flat, arr.shape)

    # 创建一个全为 NaN 的同形矩阵
    modified_arr = np.full(arr.shape, np.nan)

    # 在相应位置设置原数组中的前 30% 的元素
    modified_arr[top_30_indices_2d] = arr[top_30_indices_2d]
