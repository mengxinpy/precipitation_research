...
all_data = np.load(f'{path_out}\\result_klag_1deg_6area.npy')
...
gap = np.zeros((all_data.shape[0] - 2, all_data.shape[1]))
memory = np.zeros((all_data.shape[0] - 2, all_data.shape[1]))

top30_many = all_data[1]
all_many = all_data[0]
assert all_data.shape[1] == 6
...
for i in range(all_data.shape[1]):
    plt.plot(log_points, memory[:, i], '.', color=colors[i * 17], markersize=10)
plt.title(f"memory")
plt.ylabel('memory')
plt.xlabel(f'time')
plt.xlim(1, 180)
...
plt.xscale('log')
plt.yscale('log')
plt.savefig(f'.\\temp_fig\\ear5_lag_area\\ear5_gap_lag_40years_time-many-1deg-6area.png')
plt.close()
data = memory
