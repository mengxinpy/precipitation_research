import numpy as np
from AdaptiveWorkflow_duration import point_path_data
import xarray as xr
from Function_common import get_refine_da_list
from Graphics import scatter_plots_depart, plt_duration_origin, plt_duration_combined, scatter_plots_combined_final, scatter_plots_combined_verify_turning_points
from Function_common import depart_ml_lat


from Graphics import single_scatter_plot
from Graphics import plt_duration
from AdaptiveWorkflow_duration import load_data
...
# w850 = w850.rename('CAPE')/100
# cape_frequency = cape_frequency.rename('Vertical velocity')/100
# land_sea_mask=xr.open_dataarray(f'era5_land_sea_mask.nc').coarsen(longitude=4,latitude=4,boundary='trim').mean().sel(latitude=slice(60,-60))
# w850= w850.where(land_sea_mask == 0, np.nan).squeeze()
