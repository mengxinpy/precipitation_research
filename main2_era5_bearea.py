...
rainfall_frequency = np.squeeze(np.load('_wetday_40year.npy')) / (365 * 40 + 11)
...
ds = xr.open_mfdataset(path + '*_day_processed.nc')
n = len(ds.time)
step = n // 20

for i in range(20):
    print('area:', i)
    subset = ds.isel(time=slice(i * step, (i + 1) * step if i < 19 else n))
    # subset_values=np.squeeze(subset['precipitationCal'].values)
    # your processing here...
    for j in range(len(bins)):
        print(j)
        # rainfall_in_this_area = subset_values
        rainfall_in_this_area = subset.to_array().values.squeeze()[:, indices == j + 1].flatten() * 1000
        ...
        # save processed data
        np.save(f'.\\temp_data_era5\\era5_processed_data_area{j}_part{i}.npy', rainfall_in_this_area)
