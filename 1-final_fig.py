...
from AdaptiveWorkflow_duration import main_process, load_data
from Function_common import get_list_form_onat
from GlobalConfig import intern_data_path
from Graphics import multi_scatter_plot, plt_duration_origin, plt_duration_gamma, scatter_plots_combined_no_rect, scatter_plots_combined_final
from Function_common import get_dataset_duration_all
from GlobalConfig import path_png
import numpy as np
from Function_common import depart_ml_lat, point_path_data
from Function_common import get_refine_da_list
from Graphics import scatter_plots_combined_with_map, scatter_plots_depart, pt_single, pt_6_combine, scatter_plots_combined_with_map_new
from Graphics_Wdp import plot_map, plot_map_1, plot_distribution, combined_plot_separate_saves, plot_time_series, plot_time_series_origin
import xarray as xr
from Graphics import plt_duration_hist
from Graphics_Wdp import combined_plot
from Graphics import multi_threshold_scatter_plot, single_scatter_plot
from Processor_percentile_core import get_duration_frequency, get_all_duration_frequency

data_set_list = ['era5', 'mswep', 'persiann']
data_set = data_set_list[2]
...
# grid_list_origin = [(-67, 0), (150, 5), (0, -55), (-120, -45), (-74, 50), (110, -24)]
# #
onat_list_fig1 =grid_list4
# #
# # data_percentile, bins, indices = load_data(f'internal_data/wet_era5/wet_data.npz')
# # era5_frequency = xr.open_dataset('internal_data/era5_frequency_processed.nc').to_array().squeeze()
# # # main_process('wet',renew='000',return_point='percentile_show')
# # plot_map_1(data_frequency=era5_frequency,colorbar_title='wet-day frequency(%)',sp_fp=path_png)
# # plot_map(data_frequency=era5_frequency,dr=dr,sp_fp_map=path_png,onat_list=onat_list_fig1,all_th=all_th)
plot_time_series_origin(dr=dr,sp_fp_ts=path_png+'6ts.svg',onat_list=onat_list_fig1,all_th=all_th)
# plot_time_series_origin(dr=dr,sp_fp_ts=path_png+'6ts.svg',onat_list=grid_list_origin,all_th=all_th)

# # plot_distribution(data_percentile=data_percentile,data_frequency=era5_frequency,sp_fp=path_png)
# onat=(-120.26, -4.78)
# th=all_th.sel(longitude=onat[0],latitude=onat[1],top_bins=30,method='nearest')
# dr_list=dr.sel(longitude=onat[0],latitude=onat[1],method='nearest')
# pt_single(th=th,dr=dr_list,sp=path_png)
# combined_plot(era5_frequency, dr, 'combined_plot', onat_list=onat_list_fig1, all_th=all_th)
# combined_plot_separate_saves(data_frequency=era5_frequency,dr=dr,
#                              sp_fp_map='combined_plot_separate_saves',sp_fp_ts='combined_plot_separate_ts',
#                              onat_list=onat_list_fig1,all_th=all_th)
# %% fig2
...
from Function_common import get_refine_da_list
from Function_common import depart_ml_lat
...
# wind_10 = xr.open_dataarray('/Users/kustai/PycharmProjects/ERA5/data_0.nc').coarsen(longitude=4, latitude=4, boundary='trim').mean().sel(latitude=slice(60, -60))
# power = dataset['power'].rename('Yearly spectrum')
...
# %%
# slope = kb_mid.loc['duration_intensity', 'Slope']
# intercept = kb_mid.loc['duration_intensity', 'Intercept']
# print(slope, intercept)
# data_list2=[ data_list[1],w850,cape_frequency ]
# data_list3=[wet_source,w850_source,cape_frequency_source]
# scatter_plots(data_list2,save_path=f'./fig/reproduce_of_the_paper24')
# scatter_plots(data_list3,save_path=f'./fig/reproduce_of_the_paper24_source')
# slope = kb_mid.loc[idx['intensity','duration'], 'Slope']
# intercept = kb_mid.loc[idx, 'Intercept']
# kb=scatter_plots(data_list,save_path=f'./fig/unify')
