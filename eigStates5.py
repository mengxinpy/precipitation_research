...
def fill_vector(vector, layer):
    ...
    # print(layer)
    interp_func = interp1d(known_x, known_values, kind='linear', bounds_error=False, fill_value=0)
...
for a in range(0, 4):
    ...
    # 找出有大量异常值点的天数去掉
    eox_indices = np.unique(np.argwhere(np.sum(filtered_elements[0, :, :] < 0, axis=0) > 0.1 * filtered_elements.shape[1]))
    np.save('eox_5.1' + area_name_list[a], eox_indices)
    ...
    avg_vapor = np.mean(np.ma.masked_array(filled_data_vapor, mask=filled_data_vapor < 0), axis=1).reshape((-1, 1))
    avg_rain = np.mean(np.ma.masked_array(filled_data_rain, mask=filled_data_rain < 0), axis=1).reshape((-1, 1))
    ...
    filtered_normalized_vapor = square_normalize(filtered_elements_normalized_vapor).T
    filtered_normalized_rain = square_normalize(filtered_elements_normalized_rain).T
    ...
    for i in range(out_num):
        for v in ['vapor', 'rain']:
            if v == 'vapor':
                decode_out_variables = np.flip(np.flipud(decode_matrix(reduced_matrix_vapor[:, i], indices)), axis=0)
            else:
                decode_out_variables = np.flip(np.flipud(decode_matrix(reduced_matrix_rain[:, i], indices)), axis=0)  # 保存解码后的数据
                np.save('decode_' + v + str(a) + '_5.1th', decode_out_variables)
            ...
            start_lon = 120 + a * 90  # Starting longitude of the segment
            if start_lon > 180:
                start_lon -= 360
            end_lon = start_lon + 90  # Ending longitude of the segment

            # Handle the 180/-180 longitude breakpoint
            if end_lon > 180:
                end_lon -= 360  # Convert to the equivalent negative longitude

            # Create the longitude array for this segment
            # 画热度图

            if start_lon < end_lon:
                lon = np.linspace(start_lon, end_lon, global_var[1])
                c = ax.contourf(lon, lat, decode_out_variables, transform=ccrs.PlateCarree(), levels=100, cmap='RdYlBu_r')
            else:  # The segment crosses the 180/-180 longitude breakpoint
                lon1 = np.linspace(start_lon, 180, (180 - start_lon) * 4)
                lon2 = np.linspace(-180, end_lon, ((90 - (180 - start_lon)) * 4))
            ...
            # plt.subplots_adjust(left=0.5, right=1.0, bottom=0.1, top=0.9)
            ...
            save_path = 'pcaPicture5.1th\\' + area_name_list[a] + '\\' + area_name_list[a] + '_' + str(i) + 'th_' + v + '.png'
            os.makedirs(os.path.dirname(save_path), exist_ok=True)
            plt.savefig(save_path)
            plt.close()
np.save('singular_vapor_5.1.npy', singular_list_vapor)
np.save('singular_rain_5.1.npy', singular_list_rain)
