...
nc_files = [os.path.join(path, f) for f in os.listdir(path) if fnmatch.fnmatch(f, '199*.nc') and '_processed_day' not in f and 'hour' not in f]
...
for nc_file in nc_files:
    ...
    # 保存为新的.nc文件
    new_nc_file = nc_file.replace('.nc', 'hour_1.nc')
