...
dist = np.load('imerg_percentile_area.npy')
...
infile = infile.where((-60 <= infile.lat) & (infile.lat <= 60))
...
for ind, d in enumerate(dist_arr[0:100, :]):
    if str(type(d)) == "<class 'float'>":  # this statement ignores data that doesn't exist.
        ...
        plt.plot(d, np.arange(0, 100, 1), '.', color=colors[ind], markersize=15)
...
# saving
# plt.savefig(path + 'imerg_percentile_area.png')
plt.show()
