...
for y in range(start_year, end_year):
    for m in range(1, 13):  # 遍历月
        ...
        vapor = xr.open_dataset(f'{pathv}{y}{str(m).zfill(2)}.nc').sel(latitude=slice(flat, -flat))
        rainfall = xr.open_dataset(f'{pathr}{y}{str(m).zfill(2)}.nc').sel(latitude=slice(flat, -flat)) * 1000
