...
from function_wet50 import perform, era5_hour1year
...
def point_path_data_amsr2(lat):
    ...
    # 检查时间维度中是否有 NaN 值
    nan_time_indices = data_point.isnull().any(dim=['latitude', 'longitude'])

    # 使用 where 方法将整个时间维度的数据设为 NaN
    data_point = data_point.where(~nan_time_indices, other=np.nan)  # todo: 画图检查一下这个数据
    return data_point
...
def main_process(var, percentile_name, colorbar_title='Frequency (%)', module_name='function_percentile', dec=None, rd=False, renew=0, data_set='era5', wet=False, **func_kwargs):
    module = importlib.import_module(module_name)
    func = getattr(module, percentile_name)

    era5_frequency_np, fig_path, figure_title_font, func_percentile, lat_range, raw_dr, sp_frequency, sp_percentile, var = parm_set(data_set, dec, module, percentile_name, rd, var)
    ...
    if renew == 1:
        ...
        (bins,
         ...
         # valid_data_count,
         # lsp_fraction_percentile
         ) = load_function(sp=path_out + var + '.npz')
...
def parm_set(data_set, dec, module, percentile_name, rd, var):
    ...
    return era5_frequency_np, fig_path, figure_title_font, func_percentile, lat_range, raw_dr, sp_frequency, sp_percentile, var
