...
for area_num, area_data in enumerate(all_data):
    ...
    for ind, p in enumerate(area_data):
        ...
        # else:
        if ind == 0:
            plt.plot(p[:: 2], np.arange(1, 101)[:: 2], '.', color=colors[area_num], markersize=5, label='all')
            axname = 'all'
        elif ind == 1:
            ...
            plt.annotate(
                ...
                arrowprops=dict(arrowstyle='|-|', mutation_scale=2, lw=1, connectionstyle='bar,fraction=1,angle=0'),  # 使用双向箭头，中间带横线
                ha='center'
            )
