...
infile = xarray.open_dataset('F:\\liusch\\remote_project\\climate_new\\imerg5_process.nc')['precipitationCal'].sel(lat=slice(-60,60))*100
...
dist = np.load('imerg_float16_percentile.npy')
...
cont = plt.contourf(infile.lon, infile.lat, infile.transpose(), levels=20, cmap=cmap, vmin=0, vmax=100)
...
# saving
plt.savefig(path + 'imerg_float16_percentile')
