...
for y in range(start_year, end_year):
    for m in range(1, 13):  # 遍历月
        ...
        vapor = xr.open_dataset(f"{pathv}{y}{str(m).zfill(2)}{f'hour_{deg}' if deg != 0.25 else ''}.nc").sel(latitude=slice(flat, -flat))
        rainfall = xr.open_dataset(f"{pathr}{y}{str(m).zfill(2)}{f'hour_{deg}' if deg != 0.25 else ''}.nc").sel(latitude=slice(flat, -flat)) * 1000
...
np.save(f'.\\temp_data\\power{start_year}-{end_year}_{deg}_{vapor_strat}_{vapor_end}_{vapor_bins_count}_{wetday_gap}_{flat}_{time_gap}_avg_rain.npy', avg_rain)
np.save(f'.\\temp_data\\power{start_year}-{end_year}_{deg}_{vapor_strat}_{vapor_end}_{vapor_bins_count}_{wetday_gap}_{flat}_{time_gap}_count.npy', count)
np.save(f'.\\temp_data\\power{start_year}-{end_year}_{deg}_{vapor_strat}_{vapor_end}_{vapor_bins_count}_{wetday_gap}_{flat}_{time_gap}_binder.npy', binder_4th_order_cumulant)
np.save(f'.\\temp_data\\power{start_year}-{end_year}_{deg}_{vapor_strat}_{vapor_end}_{vapor_bins_count}_{wetday_gap}_{flat}_{time_gap}_var.npy', var)
# np.save(
